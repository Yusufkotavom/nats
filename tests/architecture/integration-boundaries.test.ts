...
describe("Integration boundaries", () => {
  it("prevents modules/integration from importing from app/", () => {
    const repoRoot = path.resolve(__dirname, "..", "..");
    const integrationRoot = path.join(repoRoot, "modules", "integration");
    const files = listFilesRecursively(integrationRoot).filter(isSourceFile);
